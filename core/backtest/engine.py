...
def run_backtest(
    ...
    for i, bar in enumerate(bars):
        ...
        if next_bar_open_fill and pending_order is not None:
            ...
            total_volume += pending_order.size_usd
            pending_order = None
...
def _apply_fill(
...
) -> tuple[float, float, Optional[Fill]]:
    """
    Record fill, update cash/position, close trades. Returns (cash, position_units, entry_fill).
    ...
    units = order.size_usd / fill_price if fill_price > 0 else 0.0
    ...
    return cash, position_units, entry_fill
