...
Commerce settlement assets are resolved at runtime from the deployed kernel — not configured via env vars in these docs. See Networks & contracts for where deployments are maintained upstream.

See .env.example in the repository for the full surface with inline comments.

← BNBAgent SDK overview
